...
extension RequestViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    /// [.accepted]
    var requestSections = [RequestStatus:[Request]]()
    var requestSectionsCount: Int {
        get {
            return requestSections.count
        }
    }
    
    func requestRowsFor(section:Int) -> Int {
        var rows = 0
        let requestValues = self.requestSections
        for (reqIndex, reqSection) in requestValues.enumerated() {
            if reqIndex == section {
                rows = reqSection.value.count
            }
        }
        return rows
    }
    ...
    var pendingCount: Int = 0 {
        didSet {
            ...
            if pendingCount == 0 {
                ...
                guard (self.requestSections[.pending]?.sorted(by: { $0.timeRequested <= $1.timeRequested })) != nil else { return }
            }
        }
    }
    ...
    var acceptedCount: Int = 0 {
        didSet {
            ...
            if acceptedCount == 0 {
                ...
                guard (self.requestSections[.accepted]?.sorted(by: { $0.timeRequested <= $1.timeRequested })) != nil else { return }
            }
        }
    }
    ...
    var rejectedCount: Int = 0 {
        didSet {
            ...
            if rejectedCount == 0 {
                ...
                guard (self.requestSections[.rejected]?.sorted(by: { $0.timeRequested <= $1.timeRequested })) != nil else { return }
            }
        }
    }
    ...
    var completeCount: Int = 0 {
        didSet {
            ...
            if completeCount == 0 {
                ...
                guard (self.requestSections[.complete]?.sorted(by: { $0.timeRequested <= $1.timeRequested })) != nil else { return }
            }
        }
    }
    ...
    var runningCount: Int = 0 {
        didSet {
            if runningCount == 0 {
                ...
                guard (self.requestSections[.running]?.sorted(by: { $0.timeRequested <= $1.timeRequested })) != nil else { return }
            }
        }
    }
        ...
        reference.observeSingleEvent(of: .value, with: { snapshot in
            if let snapValue = snapshot.value as? Dictionary<String, AnyObject> {
                for (key, val) in snapValue {
                    if let userValue = val as? String {
                        if key == "first_name" {
                            newRequest.firstName = userValue
                        } else if key == "imageUrl" {
                            newRequest.imageName = userValue
                        } else if key == "status" {
                            newRequest.userStatus = stringToDateTime(userValue)
                        }
                    }
                }
            }
            self.appendToRequests(request: newRequest)
        })
    }
    
    func appendToRequests(request: Request) {
    ...
    func setRequestStatusCountFrom(request: Request) {
        if let requestStatus = requestStatusString(request: request.requestStatus) {
            self.requestSections[requestStatus]?.append(request)
            switch requestStatus {
            case .accepted:
                self.acceptedCount += 1
            case .complete:
...
extension RequestViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    // TableView Sections
    func numberOfSections(in tableView: UITableView) -> Int {
        printFunc(requestSections.keys.count)
        printFunc(requestSectionsCount)
        return requestSectionsCount
    }
    
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        let requestKeys = self.requestSections.keys
        for (reqIndex, reqSection) in requestKeys.enumerated() {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let defaultCell = UITableViewCell()
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "RequestCell", for: indexPath) as? RequestCell else { return defaultCell }
        
        for (index, sectRequests) in self.requestSections.enumerated() {
            if index == indexPath.section {
                let sectValue = sectRequests.value[indexPath.row]
                cell.setupView(request: sectValue, animated: true)
                return cell
            }
        }
        return cell
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        let rows = requestRowsFor(section: section)
        printFunc(rows)
        return rows // requestSectionsCount[requestSections?.values[section]]
    }
