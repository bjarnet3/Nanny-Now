...
class FamilyPageZero: UIViewController {
...
extension FamilyPageZero {
    ...
    private func performAnimation() {
        ...
        if advertiseViewHidden {
            ...
            // SHOW VIEW
            // -----------
            UIView.animate(withDuration: 0.50, delay: 0.00, usingSpringWithDamping: 0.50, initialSpringVelocity: 0.34, options: .curveEaseOut, animations: {
                
                
            })
            
            UIView.animate(withDuration: 0.89, delay: 0.1514, usingSpringWithDamping: 0.50, initialSpringVelocity: 0.34, options: .curveEaseOut, animations: {
                
                self.postAnimation()
            })
            
            setProgress(progress: 1.0, animated: true, alpha: 0.15)
        } else {
            hapticButton(.light)
            // HIDE VIEW
            // -----------
            ...
            UIView.animate(withDuration: 0.89, delay: 0.11, usingSpringWithDamping: 0.55, initialSpringVelocity: 0.36, options: .curveEaseOut, animations: {
                
                self.preAnimation()
                
            })
            self.advertiseViewHidden = true
            setProgress(progress: 0.0, animated: true, alpha: 0.15)
        }
    }
